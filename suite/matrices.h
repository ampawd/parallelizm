...
namespace parallelizm 
{
	...
	class Matrix
	{
		private:
			...
			T _det_stupid(size_t, const Matrix<T>&) const;
	...
	T Matrix<T>::det() const
	{
		...
		return _det_stupid(n, *this);
	}

	template<class T>
	T Matrix<T>::_det_stupid(size_t n, const Matrix<T>& M) const 
	{
		T det = 0;
		if (n == 1)
		{
		...
		Matrix<T> cofactor(n - 1, n - 1);
		int s = 1;
		
		for (size_t k = 0, h = 0, v = 0; k < n; k++)
		{
			...
			det += s*M[0][k] *_det_stupid(n - 1, cofactor); s = -s;
		}
